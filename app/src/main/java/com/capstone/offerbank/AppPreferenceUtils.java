...
public class AppPreferenceUtils {
    ...
    public AppResponse getStoredData(Context mContext, String key) {
        ...
        if (data != null) {
            Gson gson = new Gson();
            Type type = new TypeToken<AppResponse>() {
            }.getType();
    ...
    public void persistApplist(Context context, AppResponse appResponse, String key) {
        ...
        if (appResponse != null && appResponse.apps != null && appResponse.apps.size() > 0) {
            editor.putLong(CACHE_TIME + key, System.currentTimeMillis());
            editor.putString(STORE_APP_DATA + key, gson.toJson(appResponse));
